...
void SpriteApp::Init()
{
	...
	my_sprite_2_.set_position(120.0f, 120.0f, 0.0f);
	my_sprite_2_.set_width(64.0f);
	my_sprite_2_.set_height(79.0f);
...
bool SpriteApp::Update(float frame_time)
{
	...
	vertical_speed_ =  20 * frame_time;
	horizontal_speed_ = 30 * frame_time;
	angular_speed_ = 20 * frame_time;
	
	rotate_angle_ += 5;
	rotate_angle_ %= 360;
	radians_rotate_ = (rotate_angle_ * PI) / 180;


	// get current position of the sprite
	gef::Vector4 sprite_position = my_sprite_.position();

	sprite_position.set_x( sprite_position.x() + horizontal_speed_ );
	sprite_position.set_y(sprite_position.y() + vertical_speed_);
	sprite_position.set_x(sprite_position.x() * cos(radians_rotate_));
	sprite_position.set_y(sprite_position.y() * sin(radians_rotate_));
...
void SpriteApp::DrawHUD()
{
	if(font_)
	{
		...
		font_->RenderText(
			...
			gef::Vector4(450.0f, 510.0f, -0.9f),	// position on screen
			1.0f,									// scale
